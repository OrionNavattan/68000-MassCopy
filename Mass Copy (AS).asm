srcreg:		reg a0
destreg:	reg a1
	outradix	10	; use decimal numbers when converting integers to strings
...
lblnum		:= "\{c}"			; number used in label

MassCopy_{lblnum}:	label *
...
; example usage, assuming source and destination are a0 and a1:
;	lea source(pc),a0
;	lea (destination).w,a1
;	jsr	(MassCopy).l

; uses d1.l (byte moves only), d3.w, d4.l, d5.l
; -------------------------------------------------------------------------
...
MassCopy_FinishBytes:
	...
	outradix 16		; restore default outradix
